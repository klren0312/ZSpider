    ...
    <div class="config-container">
      <div class="config-title">
        ...
        <div class="ctrl-block">
          <el-progress
            class="spider-progress"
            :width="30"
            :stroke-width="2"
            type="circle"
            :percentage="spiderProgress"
          ></el-progress>
          <el-button type="primary" size="mini" @click="start" v-if="!isStart"
            >开始采集</el-button
          >
          <el-button type="danger" size="mini" @click="end" v-else
            >停止采集</el-button
          >
          <el-button v-if="!showConfig" type="text" @click="showConfig = true"
            >展开</el-button
          >
          <el-button v-else type="text" @click="showConfig = false"
            >收起</el-button
          >
        </div>
      </div>
      ...
      <el-table :data="resultTable" border height="500px">
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="copy(scope.row.sourceUrl)"
              >复制</el-button
            >
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    async goToSpider() {
      ...
      for (let i = 1; i <= parseInt(this.config.page); i++) {
        ...
        const urls = await page.$$eval(this.config.linkRule, (a) =>
          a.map((v) => {
            return v.href
          })
        )
